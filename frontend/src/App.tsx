...
function App() {
  ...
  const [inventoryCorrectionSavingId, setInventoryCorrectionSavingId] = useState<number | null>(null);
  ...
  useEffect(() => {
    if (activeSection === "product") {
      ...
      inventoryProductRef.current?.focus();
    }
  }, [activeSection]);
  ...
  useEffect(() => {
    if (loggedIn && selectedInventorySessionId) {
      loadInventoryCounts(selectedInventorySessionId);
    }
...
if (alreadyCounted) {
  ...
  const handleApplyInventoryCorrection = async (count: InventoryCount) => {
    ...
    try {
      ...
      const response = await apiFetch(`/inventory-api/inventory-counts/${count.id}/apply-correction/`, {
        method: "POST",
      });
      ...
      <div style={{ maxWidth: "1180px", margin: "0 auto" }}>
        ...
        >
          <div>
            ...
            <h1 style={{ margin: 0, fontSize: "2.5rem", lineHeight: 1.1, color: "#f5f7fa" }}>
        ...
        >
          ...
          <Card title="Niedriger Bestand" value={String(lowStockProducts.length)} danger />
  ...
  return (
    <div
      style={{
        ...
        border: danger ? "1px solid rgba(248, 113, 113, 0.25)" : "1px solid rgba(148, 163, 184, 0.2)",
        borderRadius: "18px",
        padding: "20px",
        boxShadow: "0 10px 30px rgba(0,0,0,0.25)",
      }}
    >
      <div style={{ color: danger ? "#fca5a5" : "#94a3b8", marginBottom: "8px" }}>{title}</div>
      <div style={{ fontSize: "2rem", fontWeight: 700, color: danger ? "#fecaca" : "#f8fafc" }}>{value}</div>
    </div>
  );
}
